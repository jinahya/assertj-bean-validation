...
final class Utils {
    ...
    // -----------------------------------------------------------------------------------------------------------------
    private static final Map<String, Class<?>> SUFFIXES_AND_CLASSES = Collections.unmodifiableMap(new WeakHashMap<>());
    ...
    static <R> R applyClassForSuffix(final String suffix, final Function<? super Class<?>, ? extends R> function) {
        ...
        return function.apply(SUFFIXES_AND_CLASSES.computeIfAbsent(new String(suffix), s -> {
            if (SpiUtils.isProvidedByJakarta()) {
                try {
                    return classJakarta(s);
                } catch (final ClassNotFoundException cnfe) {
                    // empty;
                }
            }
            if (SpiUtils.isProvidedByJavax()) {
                try {
                    return classJavax(s);
                } catch (final ClassNotFoundException cnfe) {
                    // empty;
