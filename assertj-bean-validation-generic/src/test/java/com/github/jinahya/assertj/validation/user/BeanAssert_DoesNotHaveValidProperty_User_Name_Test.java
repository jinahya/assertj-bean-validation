...
@Slf4j
class BeanAssert_DoesNotHaveValidProperty_User_Name_Test {

    @DisplayName("assertBean(ofInvalidName).doesNotHaveValidProperty(\"name\") succeeds")
    @ParameterizedTest
    @ArgumentsSource(UserBeanArgumentsProviders.OfInvalidName.class)
    void doesNotHaveValidProperty_Succeed_InvalidName(
            @ConvertWith(BeanAssertArgumentConverter.class) final BeanAssert a) {
        assertThatCode(() -> a.doesNotHaveValidProperty("name"))
                .doesNotThrowAnyException();
    }

    @DisplayName("assertBean(valid).doesNotHaveValidProperty(\"name\") fails")
    @ParameterizedTest
    @ArgumentsSource(UserBeanArgumentsProviders.OfValid.class)
    void doesNotHaveValidProperty_Fail_Valid(
            @ConvertWith(BeanAssertArgumentConverter.class) final BeanAssert a) {
        assertThatThrownBy(() -> a.doesNotHaveValidProperty("name"))
